...
The evaluation suite also isolates the learning-rate schedule itself by comparing the reduce-on-plateau schedule with a fixed learning rate. Both runs use the same 7,050-image control subset, initial learning rate 0.004, Dense-64 classifier, best-checkpoint selection, and early-stopping rule. This experiment is important because the initial learning-rate comparison changes two effects at once: the starting step size and the later decayed step sizes. After rerunning this comparison, the 0.004 decay result replaces the previous 0.008-based schedule comparison.
...
\midrule
Reduce on plateau & From rerun & From rerun & From rerun & From rerun \\
Fixed learning rate & From rerun & From rerun & From rerun & 0 \\
\bottomrule
\end{tabular}
\end{table}
...
\reportfigure[0.9\linewidth]{results/baseline/evaluation/confusion_matrix_counts.png}
